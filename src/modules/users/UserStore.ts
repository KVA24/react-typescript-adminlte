...
class UserStore {
	...
	@action async getUsersAccess() {
        const result = await userService.getUsersAccess();
        if (result.status === HttpStatusCode.OK) {
            this.userList = result.body.data;
            this.totalPages = result.body.metadata.totalPages;
        } else {
    ...
    async created(isUserAccess: boolean = false){
        ...
        if(!userName) {
            ...
            const res = await userService.addUser(data, isUserAccess);
            if (res.status === HttpStatusCode.OK) {
				if(isUserAccess) {
					this.getUsersAccess();
				} else {
